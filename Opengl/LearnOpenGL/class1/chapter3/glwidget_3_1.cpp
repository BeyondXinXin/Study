...
GLWidget_3_1::GLWidget_3_1(QWidget *parent)
...
void GLWidget_3_1::initializeGL()
{
    initializeOpenGLFunctions();

    // 着色器
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shapes_3_1.vert");
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shapes_3_1.frag");
    if (!gl_shader_.link()) {
...
void GLWidget_3_1::resizeGL(int w, int h)
{
    Q_UNUSED(w)
    Q_UNUSED(h)
}

void GLWidget_3_1::paintGL()
{
...
void GLWidget_3_1::SetShape(const GLWidget_3_1::Shape &shape)
{
    shape_ = shape;
    update();
}

void GLWidget_3_1::SetWireFrame(const bool &shape)
{
    makeCurrent();
    if (shape) {
        // 线框填充方式
