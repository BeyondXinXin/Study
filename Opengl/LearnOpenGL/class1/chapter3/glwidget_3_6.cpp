...
GLWidget_3_6::GLWidget_3_6(QWidget *parent)
...
void GLWidget_3_6::initializeGL()
{
    initializeOpenGLFunctions();

    // 着色器
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shapes_2_6.vert");
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shapes_2_6.frag");
    if (!gl_shader_.link()) {
...
void GLWidget_3_6::resizeGL(int w, int h)
{
    Q_UNUSED(w)
    Q_UNUSED(h)
}

void GLWidget_3_6::paintGL()
{
...
void GLWidget_3_6::SetShape(const GLWidget_3_6::Shape &shape)
{
    shape_ = shape;
    update();
}

void GLWidget_3_6::SetWireFrame(const bool &shape)
{
    makeCurrent();
    if (shape) {
        // 线框填充方式
