...
GLWidget_2_3::GLWidget_2_3(QWidget *parent)
...
void GLWidget_2_3::initializeGL()
{
    ...
    // 着色器
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shapes_2_3.vert");
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shapes_2_3.frag");
    if (!gl_shader_.link()) {
...
void GLWidget_2_3::resizeGL(int w, int h)
{
    Q_UNUSED(w)
    Q_UNUSED(h)
}

void GLWidget_2_3::paintGL()
{
...
void GLWidget_2_3::SetShape(const GLWidget_2_3::Shape &shape)
{
    shape_ = shape;
    update();
}

void GLWidget_2_3::SetWireFrame(const bool &shape)
{
    makeCurrent();
    if (shape) {
        // 线框填充方式
