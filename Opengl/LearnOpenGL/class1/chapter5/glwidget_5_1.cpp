...
GLWidget_5_1::GLWidget_5_1(QWidget *parent)
  : QOpenGLWidget(parent)
{
    ...
    connect(&timer_, &QTimer::timeout, this, &GLWidget_5_1::AutoChange);

    setFocusPolicy(Qt::StrongFocus);
}

void GLWidget_5_1::initializeGL()
{
    initializeOpenGLFunctions();

    // 着色器
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shapes_5_1.vert");
    gl_shader_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shapes_5_1.frag");
    if (!gl_shader_.link()) {
...
void GLWidget_5_1::resizeGL(int w, int h)
{
    Q_UNUSED(w)
    Q_UNUSED(h)
...
void GLWidget_5_1::keyPressEvent(QKeyEvent *event)
{
...
void GLWidget_5_1::showEvent(QShowEvent *event)
{
    timer_.start(200);
    QOpenGLWidget::showEvent(event);
}

void GLWidget_5_1::SetShape(const GLWidget_5_1::Shape &shape)
{
    shape_ = shape;
    update();
}

void GLWidget_5_1::SetWireFrame(const bool &shape)
{
    makeCurrent();
    if (shape) {
        // 线框填充方式
...
void GLWidget_5_1::ChangeMixValue()
{
    if (mix_vlaue_ > 1.0f) {
        mix_vlaue_ = 1.0f;
    }
